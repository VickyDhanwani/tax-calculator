...
var old_regime_tax = 0;
var new_regime_tax = 0;
var old_regime_slab = 0;
var new_regime_slab = 0;
...
CalculateTax = () => {
	var output_string = "";
	this.income = parseInt(document.getElementsByClassName("income-value")[0].value);
	this.total_deductions = parseInt(document.getElementsByClassName("deductions-value")[0].value);
	...
	if(tax_regime === options[0]) {
		...
		this.old_regime_tax = this.tax_calculated;
		this.old_regime_slab = slab_index;
		output_string = "For Old Tax Regime <br>";
		output_string += "Your Taxable Income is " + t_income + "<br>"
		output_string += "You fit under " + old_tax_regime_slabs[slab_index] + " tax slab. <br>";
		output_string += "Tax Calculated is <b>" + this.old_regime_tax + "</b>."
		document.getElementsByClassName("old-regime-output")[0].innerHTML = output_string;
	}
	this.tax_calculated = 0;
	tax_regime = options[1];
	if(tax_regime === options[1]) {
		i = 0;
		t_income = income;
		tax_calculated = 0;
		slab_index = 0;
		...
		//console.log("Total calculated tax under new regime is : ", tax_calculated);
		this.new_regime_tax = this.tax_calculated;
		this.new_regime_slab = slab_index;
		output_string = "For New Tax Regime <br>";
		output_string += "Your Taxable Income is " + t_income + "<br>"
		output_string += "You fit under " + new_tax_regime_slabs[slab_index] + " tax slab. <br>";
		output_string += "Tax Calculated is <b>" + this.new_regime_tax + "</b>."
		document.getElementsByClassName("new-regime-ouput")[0].innerHTML = output_string;
	}	
}
